...
FOR /F "usebackq tokens=*" %%a IN (
    ...
    IF "%%a"=="wrapperUrl" SET WRAPPER_URL=%%a
)

@REM Extension to allow automatically downloading the maven-wrapper.jar from Maven-central
@REM This allows using the maven wrapper in projects that prohibit checking in binary data.
if exist "%WRAPPER_JAR%" goto runner
echo.
...
echo Downloading Maven wrapper...
echo.
%MAVEN_JAVA_EXE% -jar %WRAPPER_JAR% %WRAPPER_LAUNCHER% %MAVEN_PROJECTBASEDIR% %MAVEN_CONFIG%
...
if %ERROR_CODE% equ 0 goto end
echo.
echo ERROR: Maven execution finished with exit code %ERROR_CODE%. >&2
echo.
goto error
